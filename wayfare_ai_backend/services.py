...
def _format_structured_content(value: Any) -> str:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            title = title.replace("1 ", "1. ").replace("2 ", "2. ").replace("3 ", "3. ")
            body = _format_structured_content(item)
            if body:
    ...
    if isinstance(value, list):
        lines = []
        for item in value:
            item_text = _format_structured_content(item).strip()
            if item_text:
...
def _normalize_model_content(raw_content: Any) -> str:
    if isinstance(raw_content, (dict, list)):
        return _format_structured_content(raw_content).strip()
    ...
    if parsed_value is not None:
        formatted = _format_structured_content(parsed_value).strip()
        if formatted:
            return formatted
...
async def handle_annotate(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        content = _normalize_model_content(response.get("content", ""))
